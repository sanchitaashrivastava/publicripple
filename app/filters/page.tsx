...
import { Heart, ThumbsDown, ExternalLink, RefreshCw, BadgeCheck, Newspaper } from "lucide-react"
...
export default function FiltersPage() {
  ...
  useEffect(() => {
    async function loadArticles() {
      // New implementation - fetch articles once and use for all tabs
      setLoading({
        comfort: true,
        balanced: true,
        challenge: true,
      ...
      try {
        ...
        // Use the same articles for all tabs for now
        const articlesForAllTabs = {
          comfort: newsArticles,
          balanced: newsArticles,
          challenge: newsArticles,
        }

        setOriginalArticles(articlesForAllTabs)
        setArticles(articlesForAllTabs)
      } catch (error) {
  ...
  return (
    <div className="container mx-auto max-w-4xl py-8">
      <h1 className="mb-8 text-3xl font-bold">News Filters</h1>
              ...
              {articles[activeTab].map((article, index) => (
                ...
                >
                  <div onClick={() => openArticle(article.url)} className="cursor-pointer">
                    ...
                    {/* Image section - now using placeholder images */}
